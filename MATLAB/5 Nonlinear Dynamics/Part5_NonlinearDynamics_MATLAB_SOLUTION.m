...
Solow_T_CN = solow_simulate_SOLUTION(k_0, alpha, s, gamma, delta, T, @solow_cn_1step_SOLUTION);
...
for exponent = 2:8
    ...
    p_rk = cobweb_simulate(p_rk, T, n, @cobweb_rk_1step_SOLUTION);
...
step_func = @system_rk_1step_SOLUTION; % define a new name here, so it is easier to change the method for every point at once
...
for i = 1:numel(z1_ax)
    ...
    for j = 1:numel(z2_ax)
        z2_ = z2_ax(j);
        dz = system_derivative_SOLUTION([z1_; z2_]);
        dz1(i, j) = dz(1);
        dz2(i, j) = dz(2);
    end
end
